...
1 class="title-font text-base mb-2 font-bold tracking-wider text-start">{item.title}</h1>

		<div class=" flex  md:flex-row flex-col items-start cursor-pointer bg-opacity-20 hover:bg-slate-400 dark:bg-opacity-40 rounded-xl transition-all" on:click={() => DetailsPage(item.id)}>
			...
			>
				...
				<h1 class="title-font text-base mb-2 font-bold tracking-wider text-start ">{item.title.slice(0,20)}...</h1>
				{:else}
